const GAME_CONSTANTS = {
    ...
    BLACKSMITH_UPGRADE_VALUES: { RARE: [3, 4], COMMON: [1, 2] },
    ...
    LOST_COLLECTOR_XP_REWARD: { BASE_PERCENT_OF_NEXT_LEVEL: 0.3, PER_LEVEL_PERCENT: 0.005 },
    LOST_COLLECTOR_GEAR_BONUS_BASE: 5,
    LOST_COLLECTOR_GEAR_BONUS_PER_4_LEVELS: 1,
};

// NEW Game Settings State
let gameSettings = {
    numberFormat: "standard", // standard, shortened, scientific
    theme: "normal", // normal, light
};
...
const enemies = [
    ...
    },
    {
        ...
        isSpecial: true,
        lootTable: [
            { type: "weapon", chance: 0.9, rarities: ["Epic", "Legendary", "Mythic", "Divine"] },
            { type: "armor", chance: 0.9, rarities: ["Epic", "Legendary", "Mythic", "Divine"] },
        ],
    },
    {
        ...
        lootTable: [
            ...
            { type: "weapon", chance: 0.15, rarities: ["Transcendent"], isForgable: true },
        ],
    ...
    },
    {
        ...
        lootTable: [
            ...
            { type: "armor", chance: 0.15, rarities: ["Transcendent"], isForgable: true },
        ],
        potionDropChance: 0.68,
    },
    {
        ...
        lootTable: [
            ...
            { type: "weapon", chance: 0.25, rarities: ["Transcendent"], isForgable: true },
        ],
    ...
    },
    {
        ...
        lootTable: [
            ...
            { type: "armor", chance: 0.12, rarities: ["Transcendent"], isForgable: true },
        ],
...
const itemStatsByRarity = {
    /* This data remains the same */ Common: {
        weaponBonus: [1, 3],
        armorBonus: [1, 3],
    ...
    Immortal: {
        weaponBonus: [65, 80],
        armorBonus: [65, 80],
        namePrefixes: ["Immortal", "God-Slayer's", "Unending", "Reality-Render's", "Apex"],
    },
    Transcendent: {
        weaponBonus: [90, 110],
        armorBonus: [90, 110],
...
const baseItemTypes = {
    /* This data remains the same */ weapon: [
        "Dagger",
        "Shortsword",
        "Mace",
        "War Axe",
...
const purchaseConfirmationModalEl = document.getElementById("purchase-confirmation-modal");
...
const closeSettingsModalButtonEl = document.getElementById("close-settings-modal-button");
...
// NEW Number Formatting Functions
...
function formatShortened(number) {
    ...
    const suffixes = ["", "K", "M", "B", "T", "Q", "Qt", "Sx", "Sp"]; // Added more up to Septillion
    const i = Math.floor(Math.log10(Math.abs(number)) / 3);
    if (i >= suffixes.length) return number.toExponential(2); // Fallback for very large numbers
    const numStr = (number / Math.pow(1000, i)).toFixed(2);
    // Remove .00, and ensure that numbers like 1.20 become 1.2
    return (
        numStr
            .replace(/\.00$/, "")
            .replace(/(\.\d*?)0+$/, "$1")
...
function formatNumber(number) {
    switch (gameSettings.numberFormat) {
        case "shortened":
            return formatShortened(number);
        case "scientific":
            return formatScientific(number);
        case "standard":
        default:
            return formatStandard(number);
    }
}

// NEW Theme Application Function
function applyTheme(theme) {
    document.body.classList.remove("light-theme", "normal-theme"); // normal-theme isn't strictly needed if default is normal
    if (theme === "light") {
        document.body.classList.add("light-theme");
    } else {
        // document.body.classList.add('normal-theme'); // Or just rely on default body styles
    }
...
// NEW Function to load and apply settings (e.g., from localStorage)
function loadAndApplySettings() {
    ...
    if (savedTheme) gameSettings.theme = savedTheme; // Apply theme before UI updates that might depend on it
    ...
    // Initial UI update based on loaded settings
...
function updateAccessoryDescription(accessory) {
    ...
    for (const key in accessory.bonuses) {
        ...
        switch (key) {
            case "flatHp":
                parts.push(`+${formatNumber(value)} Max HP`);
                break;
            case "flatAttack":
                parts.push(`+${formatNumber(value)} ATK`);
                break;
            case "flatDefense":
                parts.push(`+${formatNumber(value)} DEF`);
                break;
            case "goldFind":
...
function updatePlayerStatsUI() {
    ...
    playerXpEl.textContent = `${formatNumber(player.xp)} / ${formatNumber(player.xpToNextLevel)}`;
    ...
    playerHpEl.textContent = `${formatNumber(player.currentHp)} / ${formatNumber(player.maxHp)}`;
...
function toggleCombatUI(isCombat) {
    /* ... (no change here) ... */ inCombat = isCombat;
...
function toggleMerchantUI(isMerchant) {
    /* ... (no change here) ... */ inMerchantEncounter = isMerchant;
...
function generateItem(itemType, possibleRarities, uniqueItemData = null) {
    /* ... (no change here, but internal bonus numbers are not yet formatted by default) ... */ if (
        uniqueItemData &&
        uniqueItemData.isUnique
    ) {
        return {
            ...
            attackBonus:
                ...
                uniqueItemData.uniqueItemType === "weapon"
                    ...
                    : uniqueItemData.uniqueItemType === "weapon" && !uniqueItemData.uniqueBonusType
    ...
    const baseName =
        baseItemTypes[itemType][Math.floor(Math.random() * baseItemTypes[itemType].length)];
    const prefix =
        rarityInfo.namePrefixes[Math.floor(Math.random() * rarityInfo.namePrefixes.length)];
    return {
...
function tryEquipItem(newItem) {
    /* ... (no change here, log messages that show numbers might need formatNumber) ... */ let equipped = false;
    ...
    if (newItem.type === "weapon") {
        ...
        if (
            ...
            addLog(
                `Equipped new weapon: <span class="rarity-${newItem.rarity}">${newItem.name}</span> (+${formatNumber(newWeaponEffectiveAttack)} ATK${newWeaponDefense > 0 ? `, +${formatNumber(newWeaponDefense)} DEF` : ""})!`,
            );
            equipped = true;
        }
        ...
        if (newItem.bonus > player.armor.bonus) {
            ...
            addLog(
                `Equipped new armor: <span class="rarity-${newItem.rarity}">${newItem.name}</span> (+${formatNumber(newItem.bonus)} DEF)!`,
            );
            equipped = true;
        }
    }
        ...
        if (newItem.type === "weapon") {
            let effectiveNewAtk =
                ...
                (newItem.bonus && !(newItem.attackBonus > 0 && newItem.defenseBonus > 0)
                    ? newItem.bonus
                    : 0);
            itemStatsLog = `+${formatNumber(effectiveNewAtk)} ATK`;
            if (newItem.defenseBonus)
                itemStatsLog += `, +${formatNumber(newItem.defenseBonus)} DEF`;
        } else {
            itemStatsLog = `+${formatNumber(newItem.bonus)} DEF`;
        }
        addLog(
...
function findRestHouse() {
    /* ... (log messages that show numbers might need formatNumber) ... */ player.currentHp =
        player.maxHp;
    addLog(
    ...
    if (itemRarity) {
        ...
        if (item.type === "weapon") {
            ...
            itemBonusText = `+${formatNumber(effectiveAtk)} ATK`;
            if (item.defenseBonus) itemBonusText += `, +${formatNumber(item.defenseBonus)} DEF`;
        } else {
            itemBonusText = `+${formatNumber(item.bonus)} DEF`;
        }
        addLog(
...
function findBlacksmith() {
    /* ... (log messages that show numbers might need formatNumber) ... */ addLog(
    ...
    if (upgradeChoice < 0.33) {
        ...
        if (
            player.weapon.name === GAME_CONSTANTS.INITIAL_PLAYER_STATS.DEFAULT_WEAPON.name &&
        ...
        addLog(
            `The blacksmith hammers away at your <span class="rarity-${player.weapon.rarity}">${player.weapon.name}</span>. Its ${enhancedStat} feels more potent! (<strong>Enhanced by ${formatNumber(upgradeAmount)}</strong>)`,
        );
    } else if (upgradeChoice < 0.66) {
        player.armor.bonus += upgradeAmount;
        addLog(
            `The blacksmith reinforces your <span class="rarity-${player.armor.rarity}">${player.armor.name}</span>. It feels sturdier! (<strong>+${formatNumber(upgradeAmount)} DEF</strong>)`,
        );
    } else {
        if (
            !player.accessory ||
            ...
            if (bonusKeys.length > 0) {
                ...
                if (percentageBonusKeys.includes(chosenBonusKey)) {
                    actualUpgradeValue = upgradeAmount * 0.01;
                ...
                let upgradeDisplayValue = percentageBonusKeys.includes(chosenBonusKey)
                    ...
                    : `${formatNumber(actualUpgradeValue)}`;
                addLog(
...
function findPotionCrate() {
    /* ... (log messages that show numbers might need formatNumber) ... */ addLog(
        "You stumble upon an abandoned crate!",
    );
    const potionsFound =
    ...
    if (potionsTaken > 0) {
        ...
        addLog(
            `You find <strong>${formatNumber(potionsTaken)} Healing Potion${potionsTaken > 1 ? "s" : ""}</strong> inside!`,
        );
    } else {
...
function encounterLostCollector() {
    /* ... (no change here) ... */ const collectorTemplate = enemies.find(
        e => e.name === "Lost Collector",
    );
    currentEnemy = {
        ...collectorTemplate,
...
function showPurchaseConfirmation(offer) {
    /* ... (cost needs formatting in message) ... */ pendingPurchaseAction = offer.action;
    const tempDiv = document.createElement("div");
    tempDiv.innerHTML = offer.description;
    confirmationMessageEl.innerHTML = `Purchase ${offer.description} for ${formatNumber(offer.cost)} Gold?`;
    purchaseConfirmationModalEl.style.display = "flex";
}
function hidePurchaseConfirmation() {
    /* ... (no change here) ... */ purchaseConfirmationModalEl.style.display = "none";
    pendingPurchaseAction = null;
}
confirmPurchaseButtonEl.addEventListener("click", () => {
    /* ... (no change here) ... */ if (pendingPurchaseAction) {
...
cancelPurchaseButtonEl.addEventListener("click", () => {
    /* ... (no change here) ... */ hidePurchaseConfirmation();
    addLog("Purchase cancelled.");
});
function calculateMerchantItemCost(item) {
    /* ... (internal calculations are fine, final cost will be formatted by UI) ... */ if (
        item.type === "accessory"
    ) {
        ...
        baseCostUnit =
            (item.cost ||
                GAME_CONSTANTS.MERCHANT_POTION_COST.BASE +
                    ...
                    player.maxHp * GAME_CONSTANTS.MERCHANT_POTION_COST.PER_MAX_HP) / 8;
    } else if (item.type === "xp_potion") {
        baseCostUnit =
            (item.cost ||
                player.xpToNextLevel *
                    (GAME_CONSTANTS.MERCHANT_XP_POTION.BASE_COST_FACTOR +
                        player.level * GAME_CONSTANTS.MERCHANT_XP_POTION.LEVEL_COST_FACTOR)) / 20;
    } else {
        baseCostUnit = (item.bonus || 0) + (item.attackBonus || 0) + (item.defenseBonus || 0);
    }
    if (baseCostUnit <= 0 && (item.type === "weapon" || item.type === "armor")) baseCostUnit = 1;
    ...
    if (item.type !== "potion" && item.type !== "xp_potion") {
        cost = cost * (1 + player.level * 0.008);
    }
    if (player.accessory && player.accessory.bonuses && player.accessory.bonuses.merchantDiscount) {
        cost = Math.floor(cost * (1 - player.accessory.bonuses.merchantDiscount));
    }
    return Math.max(item.type === "potion" || item.type === "xp_potion" ? 1 : 10, Math.floor(cost));
}
function generateMerchantOffers() {
    /* ... (xpAmount needs formatting in description, cost needs formatting in purchase message) ... */ currentMerchantOffers =
        [];
    const offerTypesPool = ["potion", "gear", "xp_potion", "accessory"];
    let availableOfferTypes = [...offerTypesPool];
    for (let i = 0; i < 2; i++) {
        ...
        let offerType;
        let accessoryOfferAttempted = currentMerchantOffers.some(o => o && o.type === "accessory");
        if (
            ...
            } else if (availableOfferTypes.includes("accessory") && !accessoryOfferAttempted) {
                offerType = "accessory";
            } else {
                currentMerchantOffers[i] = null;
                continue;
        ...
        let offer = { type: offerType, description: "", cost: 0, id: i, purchased: false };
        if (offerType === "potion") {
            offer.item = {
                type: "potion",
                cost: Math.floor(
            ...
            offer.purchaseMessage = `You bought a Health Potion for ${formatNumber(offer.cost)} Gold.`;
            offer.action = () => {
            ...
            if (player.level <= 2) {
                ...
                gearRarity =
                    r < 0.4 ? "Mythic" : r < 0.8 ? "Divine" : r < 0.95 ? "Eternal" : "Immortal";
            ...
            offer.cost = calculateMerchantItemCost(offer.item);
            let itemBonusText = `+${formatNumber(offer.item.bonus + (offer.item.attackBonus || 0))} ${offer.item.type === "weapon" ? "ATK" : "DEF"}`;
            if (offer.item.type === "weapon" && offer.item.defenseBonus) {
                itemBonusText += `, +${formatNumber(offer.item.defenseBonus)} DEF`;
            }
            offer.description = `<span class="rarity-${offer.item.rarity}">${offer.item.name}</span> (${itemBonusText})`;
            offer.purchaseMessage = `You bought <span class="rarity-${offer.item.rarity}">${offer.item.name}</span> for ${formatNumber(offer.cost)} Gold.`;
            offer.action = () => {
                player.gold -= offer.cost;
                addLog(offer.purchaseMessage);
            ...
            offer.item = {
                ...
                cost: Math.floor(
                    player.xpToNextLevel *
                        (GAME_CONSTANTS.MERCHANT_XP_POTION.BASE_COST_FACTOR +
                            player.level * GAME_CONSTANTS.MERCHANT_XP_POTION.LEVEL_COST_FACTOR),
                ),
            };
            offer.cost = calculateMerchantItemCost(offer.item);
            ...
            offer.description = `Flask of Wisdom (+${formatNumber(offer.xpAmount)} XP)`;
            offer.purchaseMessage = `You drank the Flask of Wisdom and gained ${formatNumber(offer.xpAmount)} XP!`;
            offer.action = () => {
                player.gold -= offer.cost;
            ...
            let availableAcc = accessories.filter(acc => {
                ...
                if (player.level < 10) return ["Common", "Uncommon"].includes(acc.rarity);
                if (player.level < 18) return ["Uncommon", "Rare"].includes(acc.rarity);
                if (player.level < 25) return ["Rare", "Epic"].includes(acc.rarity);
                return ["Epic"].includes(acc.rarity);
            });
            if (availableAcc.length === 0)
                availableAcc = accessories.filter(acc => acc.rarity === "Common");
            if (availableAcc.length === 0) {
                currentMerchantOffers[i] = null;
                continue;
            }
            ...
            offer.item = JSON.parse(JSON.stringify(selectedAccessoryTemplate));
            offer.cost = calculateMerchantItemCost(offer.item);
            updateAccessoryDescription(offer.item); // Ensure its description is formatted with current settings if it has numbers
            offer.description = `<span class="rarity-${offer.item.rarity}">${offer.item.name}</span> (${offer.item.description})`;
            offer.purchaseMessage = `You bought the <span class="rarity-${offer.item.rarity}">${offer.item.name}</span> for ${formatNumber(offer.cost)} Gold.`;
            offer.action = () => {
                player.gold -= offer.cost;
                if (
                    player.accessory &&
                    ...
                    player.maxHp = Math.max(1, player.maxHp - player.accessory.bonuses.flatHp);
                    player.currentHp = Math.min(player.currentHp, player.maxHp);
                    if (player.currentHp <= 0) player.currentHp = 1;
                }
                player.accessory = JSON.parse(JSON.stringify(offer.item));
                if (player.accessory.bonuses && player.accessory.bonuses.flatHp) {
                    player.maxHp += player.accessory.bonuses.flatHp;
                }
                addLog(offer.purchaseMessage);
                updateAccessoryDescription(player.accessory); // Update description of equipped accessory
                addLog(
...
function updateMerchantUI() {
    /* ... (cost needs formatting) ... */ if (!inMerchantEncounter) return;
    for (let i = 0; i < 2; i++) {
        ...
        if (offer && offer.type) {
            offerEl.innerHTML = offer.description;
            buttonEl.innerHTML = `Buy (Cost: ${formatNumber(offer.cost)} G)`;
            buttonEl.disabled = offer.purchased || player.gold < offer.cost;
            if (offer.type === "potion" && player.potions >= player.maxPotions) {
                buttonEl.disabled = true;
                if (!offer.purchased)
                    offerEl.innerHTML =
                        offer.description + " <span style='color:orange;'>(Max Potions!)</span>";
            }
            ...
            buttonEl.innerHTML = "---";
            buttonEl.disabled = true;
            if (i === 0 && (!currentMerchantOffers[1] || !currentMerchantOffers[1].type)) {
...
function encounterMerchant() {
    /* ... (no change here) ... */ addLog("You encounter a peculiar Wandering Merchant!");
    toggleMerchantUI(true);
    generateMerchantOffers();
}
function spawnEnemy() {
    /* ... (scaled stats in log need formatting, like level) ... */ let enemyTemplate;
    let enemyPool = [];
    let playerLevelAdvantage = 0;
    let isElite = false;
    const G_CONST = GAME_CONSTANTS;
    const firstEncounterOverride = firstEncounter;
    if (firstEncounter) {
        ...
        if (player.level <= 2) {
            ...
            if (Math.random() < 0.04) {
                ...
                if (bP.length > 0) enemyPool.push(bP[Math.floor(Math.random() * bP.length)]);
            }
            ...
            if (Math.random() < 0.5 && !firstEncounterOverride) {
                enemyPool = [enemies.find(e => e.name === "The Nether Tyrant") || enemyPool[0]];
            }
        }
        if (enemyPool.length === 0) {
            enemyPool = [
                enemies.find(e => e.name === "Goblin Grunt" && !e.isSpecial && !e.isFinalBoss) ||
                    enemies.find(e => !e.isSpecial && !e.isFinalBoss),
            ];
        }
        if (enemyPool.length === 0) enemyPool = [enemies[0]];
        enemyTemplate = enemyPool[Math.floor(Math.random() * enemyPool.length)];
    }
    const enemyTemplateForScaling = enemies.find(e => e.name === enemyTemplate.name);
    ...
    if (isElite) {
        ...
        currentLootTable = currentLootTable.map(loot => ({
            ...loot,
            chance: Math.min(1, loot.chance * G_CONST.ELITE_MODIFIERS.LOOT_CHANCE_MULTIPLIER),
        }));
    }
    ...
    currentEnemy = {
        ...
        attack: Math.floor(
            baseAttack +
                ...
                Math.floor(player.level * G_CONST.ENEMY_SCALING.ATK_FLAT_PER_PLAYER_LEVEL),
        ),
        defense: Math.floor(
            baseDefense +
                playerLevelAdvantage *
                    (baseDefense * G_CONST.ENEMY_SCALING.DEF_PER_LEVEL_ADVANTAGE_FACTOR) +
                Math.floor(player.level * G_CONST.ENEMY_SCALING.DEF_FLAT_PER_PLAYER_LEVEL),
        ),
        xpYield: xpYield,
    ...
    addLog(
        `A wild ${currentEnemy.isElite ? `<span class="elite-enemy">${currentEnemy.name}</span>` : `<strong>${currentEnemy.name}</strong>`} (Lvl ~${formatNumber(player.level)}) appears!${currentEnemy.isElite ? " It radiates a menacing aura!" : ""}`,
    );
    updateEnemyStatsUI();
    toggleCombatUI(true);
}
exploreButton.addEventListener("click", () => {
    /* ... (no change here) ... */ addLog("You venture deeper into the dungeon...");
...
attackButton.addEventListener("click", () => {
    /* ... (damage numbers in log need formatting) ... */ if (!currentEnemy || !inCombat) return;
    player.isDefending = false;
    const G_CONST_PLAYER = GAME_CONSTANTS.PLAYER_BASE_CRIT_CHANCE;
    ...
    if (Math.random() < critChance) {
        ...
        addLog(
            `<strong>CRITICAL HIT!</strong> You attack ${enemyDisplayName} for <strong style="color:#ff8c00;">${formatNumber(playerDamage)}</strong> damage!`,
        );
    } else {
        addLog(
            `You attack ${enemyDisplayName} for <strong>${formatNumber(playerDamage)}</strong> damage.`,
        );
    }
...
function enemyTurn() {
    /* ... (damage numbers in log need formatting) ... */ if (
        !currentEnemy ||
    ...
    const enemyCritChance =
        ...
        player.level * GAME_CONSTANTS.ENEMY_CRIT_CHANCE_LEVEL_MULTIPLIER;
    if (
        currentEnemy.attack > (currentEnemy.ignoresDefense ? -1 : effectivePlayerDefense) &&
        ...
        addLog(
            `${messagePrefix} lands a <strong style="color:#ff4500;">critical blow</strong> for <strong>${formatNumber(enemyDamage)}</strong> damage!`,
        );
    } else {
        if (currentEnemy.ignoresDefense) {
            addLog(
                `${messagePrefix} hits you for <strong>${formatNumber(enemyDamage)}</strong> damage!`,
            );
        } else {
            addLog(
                `${messagePrefix} attacks you for <strong>${formatNumber(enemyDamage)}</strong> damage.`,
            );
        }
    }
    ...
    const actualEnemyNameForAbilities = currentEnemy.isElite
        ...
        : currentEnemy.name;
    const enemyTemplateOriginal = enemies.find(e => e.name === actualEnemyNameForAbilities);
    if (enemyTemplateOriginal) {
        if (actualEnemyNameForAbilities === "Chronomancer Adept") {
            if (Math.random() < (currentEnemy.timeWarpHealChance || 0)) {
                const healAmount = Math.floor(enemyTemplateOriginal.baseHp * 0.15);
                currentEnemy.currentHp = Math.min(
                    currentEnemy.baseHp,
                    currentEnemy.currentHp + healAmount,
                );
                addLog(
                    `The ${enemyDisplayName} warps time, restoring <strong style="color:lightgreen;">${formatNumber(healAmount)}</strong> HP!`,
                );
                updateEnemyStatsUI();
            }
        }
        if (actualEnemyNameForAbilities === "Corrupted Treant" && enemyDamage > 0) {
            const leechAmount = Math.floor(enemyDamage * (currentEnemy.lifeLeechMultiplier || 0));
            if (leechAmount > 0) {
                currentEnemy.currentHp = Math.min(
                    currentEnemy.baseHp,
                ...
                addLog(
                    `The ${enemyDisplayName} leeches <strong style="color:darkgreen;">${formatNumber(leechAmount)}</strong> HP from you!`,
                );
                updateEnemyStatsUI();
            }
        }
        if (
            ...
            if (currentEnemy.ignoresDefense) {
                ...
                extraMessagePrefix += " <strong style='color:#BA55D3;'>pierces</strong>";
            } else {
                extraEnemyDamage = Math.max(1, currentEnemy.attack - extraEffectivePlayerDefense);
            }
            if (
                currentEnemy.attack >
                ...
                addLog(
                    `${extraMessagePrefix} lands a <strong style="color:#ff4500;">critical blow</strong> for <strong>${formatNumber(extraEnemyDamage)}</strong> damage!`,
                );
            } else {
                addLog(
                    `${extraMessagePrefix} hits you for <strong>${formatNumber(extraEnemyDamage)}</strong> damage.`,
                );
            }
...
function handleRegularLootDrop(defeatedEnemyNameForLogDisplay) {
    /* ... (item bonus in log needs formatting) ... */ const enemyDataForLoot = currentEnemy;
    ...
    enemyDataForLoot.lootTable.forEach(lootEntry => {
        if (Math.random() < lootEntry.chance) {
            ...
            if (lootEntry.isUnique) {
                item = generateItem(lootEntry.uniqueItemType, [lootEntry.uniqueRarity], lootEntry);
            ...
            if (item.type === "weapon") {
                ...
                itemBonusText = `+${formatNumber(effectiveAttack)} ATK`;
                if (item.defenseBonus) {
                    itemBonusText += `, +${formatNumber(item.defenseBonus)} DEF`;
                }
            ...
            if (originalLootEntry && originalLootEntry.isForgable) {
                const forgedBonus = Math.floor(Math.random() * 20) + 1;
                if (item.type === "weapon") {
                    if (item.attackBonus > 0 && item.defenseBonus > 0)
                        item.attackBonus += forgedBonus;
                    else item.bonus += forgedBonus;
                    effectiveAttack = (item.attackBonus || 0) + item.bonus;
                    if (item.attackBonus > 0 && item.defenseBonus > 0) {
                        effectiveAttack = item.attackBonus;
                    }
                ...
                item.name = `${originalName} (Forged +${formatNumber(forgedBonus)})`;
...
function handleEnemyDefeat() {
    /* ... (gold, xp, item bonus numbers in log need formatting) ... */ const defeatedEnemyNameForLog =
        currentEnemy.isElite
            ? `<span class="elite-enemy">${currentEnemy.name}</span>`
            : `<strong>${currentEnemy.name}</strong>`;
    const originalXpYield = currentEnemy.xpYield;
    const enemyDataForLoot = currentEnemy;
    const G_CONST = GAME_CONSTANTS;
    ...
    if (enemyDataForLoot && enemyDataForLoot.goldDropRange) {
        let goldDropped =
            ...
            Math.floor(
                Math.random() *
                    (enemyDataForLoot.goldDropRange[1] - enemyDataForLoot.goldDropRange[0] + 1),
            );
        if (player.accessory && player.accessory.bonuses && player.accessory.bonuses.goldFind) {
            goldDropped = Math.floor(goldDropped * (1 + player.accessory.bonuses.goldFind));
        }
        player.gold += goldDropped;
        addLog(
            `The ${defeatedEnemyNameForLog} dropped <strong style="color:#ffd700;">${formatNumber(goldDropped)} Gold</strong>!`,
        );
    }
    const actualEnemyName = currentEnemy.isElite
        ? currentEnemy.name.substring(6)
    ...
    if (actualEnemyName === "Lost Collector") {
        ...
        if (player.accessory && player.accessory.bonuses && player.accessory.bonuses.xpGain) {
            finalXpReward = Math.floor(finalXpReward * (1 + player.accessory.bonuses.xpGain));
        }
        player.xp += finalXpReward;
        addLog(
            `The Lost Collector dissolves, leaving behind a massive surge of experience! Gained <strong>${formatNumber(finalXpReward)}</strong> XP.`,
        );
        ...
        if (player.weapon.attackBonus > 0 && player.weapon.defenseBonus > 0)
            player.weapon.attackBonus += collectorBonus;
        else player.weapon.bonus += collectorBonus;
        player.armor.bonus += collectorBonus;
        addLog(
            `A strange energy enhances your gear! Your <span class="rarity-${player.weapon.rarity}">${player.weapon.name}</span> gains +${formatNumber(collectorBonus)} to its main attack stat and your <span class="rarity-${player.armor.rarity}">${player.armor.name}</span> gains +${formatNumber(collectorBonus)} DEF!`,
        );
    } else if (currentEnemy.isFinalBoss) {
        addLog("The Nether Tyrant shatters into oblivion, its dark power dispersed!");
        let finalXpYield = originalXpYield;
        if (player.accessory && player.accessory.bonuses && player.accessory.bonuses.xpGain) {
        ...
        const potionsGained =
            ...
            Math.floor(player.level / G_CONST.FINAL_BOSS_LOOT.POTIONS_PER_5_PLAYER_LEVELS);
        ...
        if (actuallyGained > 0) {
            ...
            addLog(
                `The Tyrant dropped <strong>${formatNumber(actuallyGained)} Healing Potion${actuallyGained > 1 ? "s" : ""}</strong>!`,
            );
        }
        const numHighItems =
            G_CONST.FINAL_BOSS_LOOT.HIGH_ITEM_COUNT_BASE +
            (Math.random() < G_CONST.FINAL_BOSS_LOOT.HIGH_ITEM_ADDITIONAL_CHANCE ? 1 : 0);
        for (let i = 0; i < numHighItems; i++) {
            const itemType = Math.random() < 0.5 ? "weapon" : "armor";
            const item = generateItem(itemType, ["Mythic", "Divine"]);
            addLog(
                `It dropped a pulsating <strong class="rarity-${item.rarity}">${item.name}</strong> (+${formatNumber((item.bonus || 0) + (item.attackBonus || 0))} ${item.type === "weapon" ? "ATK" : "DEF"})!`,
            );
            tryEquipItem(item);
        }
        if (Math.random() < G_CONST.FINAL_BOSS_LOOT.ETERNAL_DROP_CHANCE) {
            ...
            addLog(
                `Among the remains, an <strong class="rarity-Eternal">${item.name}</strong> (+${formatNumber((item.bonus || 0) + (item.attackBonus || 0))} ${item.type === "weapon" ? "ATK" : "DEF"}) gleams!`,
            );
            tryEquipItem(item);
        }
        if (Math.random() < G_CONST.FINAL_BOSS_LOOT.IMMORTAL_DROP_CHANCE) {
            ...
            addLog(
                `A shard of pure power materializes into an <strong class="rarity-Immortal">${item.name}</strong> (+${formatNumber((item.bonus || 0) + (item.attackBonus || 0))} ${item.type === "weapon" ? "ATK" : "DEF"})!`,
            );
            tryEquipItem(item);
        }
        if (Math.random() < G_CONST.FINAL_BOSS_LOOT.TRANSCENDENT_DROP_CHANCE) {
            ...
            addLog(
                `An echo of ultimate power forms into a <strong class="rarity-Transcendent">${item.name}</strong> (+${formatNumber((item.bonus || 0) + (item.attackBonus || 0))} ${item.type === "weapon" ? "ATK" : "DEF"})!`,
            );
            tryEquipItem(item);
        }
    } else {
        ...
        player.xp += finalXpYield;
        addLog(`Gained <strong>${formatNumber(finalXpYield)}</strong> XP.`);
        const baseEnemyTemplateForPotion = enemies.find(e => e.name === actualEnemyName);
        let effectivePotionDropChance =
            (baseEnemyTemplateForPotion && baseEnemyTemplateForPotion.potionDropChance) || 0;
        if (currentEnemy.isElite)
            effectivePotionDropChance *= G_CONST.ELITE_MODIFIERS.LOOT_CHANCE_MULTIPLIER;
        if (
            player.accessory &&
            player.accessory.bonuses &&
        ...
        if (Math.random() < effectivePotionDropChance && player.potions < player.maxPotions) {
            player.potions++;
            addLog(`The ${defeatedEnemyNameForLog} dropped a Healing Potion!`);
        }
        if (
            (actualEnemyName === "Ancient Wyrm" || actualEnemyName === "Dungeon Guardian") &&
            player.potions < player.maxPotions
        ) {
            const bossPotions = Math.floor(Math.random() * 3) + (currentEnemy.isElite ? 3 : 2);
            ...
            if (actuallyGainedBossPotions > 0) {
                ...
                addLog(
                    `The mighty ${defeatedEnemyNameForLog} dropped <strong>${formatNumber(actuallyGainedBossPotions)} Healing Potion${actuallyGainedBossPotions > 1 ? "s" : ""}</strong>!`,
                );
            }
        }
        handleRegularLootDrop(defeatedEnemyNameForLog);
        ...
        if (
            ...
            if (actualEnemyName === "Ancient Wyrm" || actualEnemyName === "Dungeon Guardian") {
                ...
                ultraLootChance = G_CONST.SPECIAL_BOSS_LOOT.HIGH_TIER_REGULAR.ULTRA_CHANCE;
                if (
                    ["Void Reaver", "Infernal Juggernaut", "Archlich Channeler"].includes(
                        actualEnemyName,
                    )
                ) {
                    transcendentLootChance = G_CONST.SPECIAL_BOSS_LOOT.VOID_ETC_TRANSCENDENT_CHANCE;
                }
            }
            if (transcendentLootChance > 0 && Math.random() < transcendentLootChance) {
                ...
                const item = generateItem(itemType, ["Transcendent"]);
                addLog(
                    `Beyond comprehension! A <strong class="rarity-Transcendent">${item.name}</strong> (+${formatNumber((item.bonus || 0) + (item.attackBonus || 0))} ${item.type === "weapon" ? "ATK" : "DEF"}) manifests from the dissipating foe!`,
                );
                tryEquipItem(item);
                ...
                addLog(
                    `The fallen foe radiates immense power, dropping an <strong class="rarity-${item.rarity}">${item.name}</strong> (+${formatNumber((item.bonus || 0) + (item.attackBonus || 0))} ${item.type === "weapon" ? "ATK" : "DEF"})!`,
                );
                tryEquipItem(item);
            }
        }
...
function checkLevelUp() {
    /* ... (level up messages need formatting for level, stats gained) ... */ let leveledUp = false;
    ...
    while (player.xp >= player.xpToNextLevel) {
        ...
        const levelConfig =
            G_CONST_LU.find(m => player.level >= m.minLevel) || G_CONST_LU[G_CONST_LU.length - 1];
        ...
        const hpGain =
            Math.floor(
                player.maxHp *
                    (G_CONST_HP.BASE_PERCENT + player.level * G_CONST_HP.LEVEL_MULTIPLIER),
            ) +
            (G_CONST_HP.FLAT_BASE + player.level);
        let attackGain =
            Math.floor(
                player.baseAttack *
                    (G_CONST_ATK.BASE_PERCENT + player.level * G_CONST_ATK.LEVEL_MULTIPLIER),
            ) +
            (player.level % 2 === 0 ? G_CONST_ATK.EVEN_LEVEL_BONUS : G_CONST_ATK.ODD_LEVEL_BONUS) +
            Math.floor(player.level / 4) * G_CONST_ATK.PER_4_LEVELS_BONUS;
        let defenseGain =
            Math.floor(
                player.baseDefense *
                    (G_CONST_DEF.BASE_PERCENT + player.level * G_CONST_DEF.LEVEL_MULTIPLIER),
            ) +
            (player.level % 3 === 0
        ...
        addLog(
            `<strong style="color: #00dd00;">LEVEL UP! You are now Level ${formatNumber(player.level)}!</strong>`,
        );
        addLog(
            `Max HP +${formatNumber(hpGain)}, Base ATK +${formatNumber(attackGain)}, Base DEF +${formatNumber(defenseGain)}. HP restored.`,
        );
    }
    if (leveledUp) updatePlayerStatsUI();
}
function handlePlayerDefeat() {
    /* ... (level in message needs formatting) ... */ addLog(
        `<strong style="color: #ff0000;">You have been defeated!</strong>`,
    );
...
function resetGame() {
    ...
    player = {
        ...
        maxPotions: INIT_STATS.MAX_POTIONS,
    ...
    loadAndApplySettings(); // Load settings first
    updateAccessoryDescription(player.accessory);
    ...
    if (settingsModalEl) settingsModalEl.style.display = "none"; // NEW: ensure settings modal is closed

    updatePlayerStatsUI(); // This will now use formatted numbers
    updateEnemyStatsUI();
    toggleCombatUI(false);
...
numberFormatRadios.forEach(radio => {
    radio.addEventListener("change", event => {
        ...
        updatePlayerStatsUI(); // Re-render numbers with new format
        if (inMerchantEncounter) updateMerchantUI(); // Also update merchant if open
        if (currentEnemy) updateEnemyStatsUI(); // Update enemy HP if visible
        // Could update log numbers too if desired, but that's more complex due to static nature of log entries
        saveSettings();
    });
});
...
buyOffer1Button.addEventListener("click", () => {
    /* ... (no change here) ... */ const offer = currentMerchantOffers[0];
...
buyOffer2Button.addEventListener("click", () => {
    /* ... (no change here) ... */ const offer = currentMerchantOffers[1];
    if (offer && offer.action && !offer.purchased && player.gold >= offer.cost) {
        if (offer.type === "potion" && player.potions >= player.maxPotions) {
            addLog("You can't carry any more potions!");
            return;
...
leaveMerchantButton.addEventListener("click", () => {
    /* ... (no change here) ... */ addLog(
        "The merchant waves goodbye and disappears into the shadows.",
    );
    toggleMerchantUI(false);
    toggleCombatUI(false);
});
...
document.addEventListener("keydown", function (event) {
    if (event.key === "Escape") {
        if (settingsModalEl && settingsModalEl.style.display === "flex") {
            // NEW: Close settings modal
            settingsModalEl.style.display = "none";
            event.preventDefault();
            return;
        } else if (purchaseConfirmationModalEl.style.display === "flex") {
    ...
    // If any modal is open, don't process game actions (except for their own close/confirm keys)
    if (settingsModalEl.style.display === "flex" || helpModalEl.style.display === "flex") return;

    if (inCombat) {
        if ((event.key === " " || event.code === "Space") && isActionable(attackButton)) {
            attackButton.click();
            event.preventDefault();
        } else if ((event.key === "d" || event.key === "D") && isActionable(defendButton)) {
            defendButton.click();
            event.preventDefault();
        ...
        if ([" ", "d", "D", "p", "P"].includes(event.key.toLowerCase()) || event.code === "Space")
            return;
    }
...
resetGame(); // Initialize game and load settings
